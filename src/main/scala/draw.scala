object draw {
  ...
  def drawCircle(centre_x: Int, centre_y: Int, radius: Int) = (radius > 0) match {
    case false => Cons((centre_x, centre_y), Nil())
    case true => drawCircleRec(centre_x, centre_y, radius, 0, () => 1 - radius, () => Nil())
  }
